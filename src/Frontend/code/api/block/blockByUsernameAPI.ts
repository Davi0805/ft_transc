...
/**
 * @brief
 * Blocks the specified user by username.
 *
 ...
 *
 */
export async function blockByUserName(
  username: string
): Promise<void> {
  try {
    if (!authService.isUserAuthenticated()) {
      const errorMessage: string = `DEBUG: No authToken at blockByUserName`;
      const error: Error = new Error(errorMessage);
      throw error;
    }

    const response = await fetch(
      `localhost:8080/friend_requests/${username}/blockbyusername`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      const errorMessage: string = `DEBUG: blockByUserName failed with status ${response.status}`;
      const error: Error = new Error(errorMessage);
