import { getSelfData, SelfData } from "../api/userData/getSelfDataAPI";
import { LobbyPage } from "../pages/play/lobby";
import { TSlots } from "../pages/play/utils/helpers";
import {
  TDynamicLobbySettings,
  TFriendlyPlayer,
  TLobby,
  TLobbyType,
  TMatchPlayer,
...
import { getUserDataById } from "../api/userData/getUserDataAPI";
...
class LobbyService {
  ...
  getSlots(): TSlots {
    if (!this._settings) {
      throw Error("getSlots should not be called before lobby is initialized!");
    }
    ...
    matchPlayers.forEach((player) => {
      if (
        mapSlots[player.team] === undefined ||
        mapSlots[player.team]![player.role] === undefined
      ) {
        throw Error("Fuck everything about this bullshit");
      }
      if (player.id === null || player.spriteID === null) {
        throw Error(
          "Player should have been initialized by backend at this point!"
        );
      }
  ...
  async joinLobbyOUT(
    settings: TLobby,
    users: { id: number; ready: boolean; host: boolean }[]
  ) {
    const selfData = await getSelfData();
    for (const user in users) {
      const userData = await getUserDataById(selfData.id);
      const player =
        settings.type === "friendly"
          ? []
          : settings.type === "ranked"
          ? { team: SIDES.LEFT, role: ROLES.BACK }
  ...
  async addLobbyUserOUT(userID: number) {
    ...
    const player =
      this.settings.type === "friendly"
        ? []
        : this.settings.type === "ranked"
        ? { team: SIDES.LEFT, role: ROLES.BACK }
        : { applied: false, score: 0, prevOpponents: [], teamDist: 0 }; //TODO: SHOULDNT THIS SHIT BE GENERATED IN BACKEND??
    this._users.push({
      id: Number(userData.user_id),
  ...
  addTournamentPlayer(userID: number) {
    if (!this._isLobbyOfType("tournament")) {
      return;
    }
    const user = this._findUserByID(userID);
    user.participating = true;
  ...
  getMatchPlayers(): TMatchPlayer[] {
    ...
    if (this._isLobbyOfType("friendly")) {
      ...
      throw Error("Function called in wrong lobby type");
    }
    return out;
  }
  ...
  isEveryoneReady(): boolean {
    return this._users.find(
      (user) => user.participating === true && user.ready === false
    )
      ? false
      : true;
  }
  amIParticipating(): boolean {
    ...
    console.log(
      `MyID from lobby is: ${this.myID} and the lobby list of users is ${this._users}`
    );
    const me = this._users.find((user) => user.id == this.myID);
    if (!me) {
      throw Error("How can't I be in the lobby???");
    }
  ...
  get settings(): TLobby {
    if (!this._settings) {
      throw Error(
        "Settings are trying to be accessed when lobby hasn't been initialized yet!"
      );
    }
  ...
  get myID(): number {
    if (this._myID === null) {
      throw Error("myID is not initialized!");
    }
    return this._myID;
  }
  ...
  _findUserByID(userID: number): TUser {
    ...
    if (!user) {
      throw Error("User requested was not found in lobby!");
    }
    return user;
  }
