...
class TournamentService {
    ...
    _displayStandings(tournamentID) {
    ...
    _startRound(tournamentID, pairingsIDs) {
        const tournament = tournamentRepository.getTournamentByID(tournamentID);
        ...
        for (let i = 0; i < pairings.length; i++) {
            ...
            if (!currentRound) {
                throw Error("The current round was not initialized");
            }
    ...
    _onMatchFinished(tournamentID, matchID, result, players) {
        const matchUsers = matchRepository.getMatchUsersByID(matchID);
        if (!matchUsers) {
            throw Error("This match does not exist!");
        }
        ...
        if (!match) {
            throw Error("This match does not exist in the tournament!");
        }
        ...
        if (!playerLeft || !playerRight) {
            throw Error("Players in match do not exist in participants!");
        }
    ...
    _displayResults(tournamentID, matchUsers, allGamesDone) {
        socketService.broadcastToUsers(matchUsers, "displayResults", null);
        if (allGamesDone) {
            //Only runs the following if this game is the last one
            setTimeout(() => {
    ...
    _getTournamentParticipants(users) {
        const out = [];
        users.forEach(user => {
            out.push({
                id: user.id,
    ...
    _updateAndGetPlayerPairingsFromIDs(players, pairingsIDs) {
        ...
        byePairings.forEach(pair => {
            ...
            if (!player) {
                throw Error("The playerID in the pairing does not exist in the player list");
            }
            player.score++;
