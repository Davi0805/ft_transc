...
class LobbyService {
    ...
    updateSettings(lobbyID, senderID, newSettings) {
        const lobby = lobbyRepository.getLobbyByID(lobbyID);
    ...
    updateUserReadinesss(lobbyID, userID, ready) {
    ...
    addFriendlyPlayer(lobbyID, userID, player) {
        const user = lobbyRepository.getLobbyUserByID(lobbyID, userID);
    ...
    removeFriendlyPlayer(lobbyID, userID, playerID) {
        const user = lobbyRepository.getLobbyUserByID(lobbyID, userID);
        user.player = user.player.filter(player => player.id !== playerID);
    ...
    addRankedPlayer(lobbyID, userID, player) {
        const user = lobbyRepository.getLobbyUserByID(lobbyID, userID);
        user.player = player;
    ...
    removeRankedPlayer(lobbyID, userID) {
        const user = lobbyRepository.getLobbyUserByID(lobbyID, userID);
        user.player = null;
    ...
    addTournamentPlayer(lobbyID, userID) {
        const lobby = lobbyRepository.getLobbyByID(lobbyID);
    ...
    removeTournamentPlayer(lobbyID, userID) {
        const user = lobbyRepository.getLobbyUserByID(lobbyID, userID);
        user.player = null;
    ...
    _currentID = 0; //Necessary to tag the friendly players
    _isEveryoneReady(lobbyUsers) {
