import type { FriendlyPlayerT, LobbyCreationConfigsT, LobbyT, LobbyTypeT, LobbyUserT, RankedPlayerT, TournamentPlayerT } from "../Factories/LobbyFactory.js";
...
class LobbyService {
    ...
    addFriendlyPlayer(lobbyID: number, userID: number, player: FriendlyPlayerT) {
        ...
        const user = lobby.users.find(user => user.id == userID); //this._getLobbyUserByID(lobbyID, userID);
        if (!user) {
            console.log("User does not exist in this lobby. Ignoring..."); //This should not ever happen, but just in case I will not throw
            return;
        }
